...
const configGame = {
  ...
  meta: () => { return typeGame == 'real' ? configGame.value * 4 : configGame.value * 4; }
}
...
function extTriggerPoints(coin = 10) {
  var currentPoints = els.currentPoints();
  var percent = typeGame == 'real'? 80 : 80;
...
function execGreen() {
    ...
    form.setAttribute('action', '/influencer/win.php');
...
function execRed() {
    ...
    var form = document.createElement('form');
    form.setAttribute('method', 'post');
    form.setAttribute('action', '/influencer/loss.php');
